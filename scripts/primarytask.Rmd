...
train_labels_binary <- training(partitions) %>%
  pull(bclass) %>%
  as.numeric() - 1 # Binary labels must be 0/1
train_labels_multiclass <- training(partitions) %>%
  pull(mclass) %>%
  as.numeric() - 1 # Multiclass labels must start at 0
...
test_labels_binary <- testing(partitions) %>%
  pull(bclass) %>%
  as.numeric() - 1
test_labels_multiclass <- testing(partitions) %>%
  pull(mclass) %>%
  as.numeric() - 1
train_text <- as.character(train_text)
test_text <- as.character(test_text)
```

```{r}
# Convert training data to TensorFlow dataset
train_dataset <- tensor_slices_dataset(list(train_text, train_labels_binary)) %>%
  dataset_batch(batch_size = 32)

# Convert testing data to TensorFlow dataset
test_dataset <- tensor_slices_dataset(list(test_text, test_labels_binary)) %>%
  dataset_batch(batch_size = 32)
```

```{r}
# Preprocessing Layer for Neural Networks
...
build_nn_model <- function(output_units, activation, loss, metrics) {
  keras_model_sequential() %>%
    preprocess_layer() %>%
    layer_dropout(0.2) %>%
    layer_dense(units = 128, activation = 'relu') %>%
    layer_dropout(0.2) %>%
    layer_dense(units = output_units, activation = activation) %>%
    compile(
      loss = loss,
...
binary_results <- binary_model %>% evaluate(x = test_text, y = test_labels_binary)
print(binary_results)

# Multiclass Classification Neural Network
num_classes <- length(unique(train_labels_multiclass))
multiclass_model <- build_nn_model(
...
multiclass_history <- multiclass_model %>%
  fit(
    x = train_text,
...
multiclass_results <- multiclass_model %>% evaluate(x = test_text, y = test_labels_multiclass)
print(multiclass_results)

# Predictions and Metrics
binary_predictions <- binary_model %>% predict(test_text) %>% as.numeric()
binary_pred_classes <- ifelse(binary_predictions > 0.5, 1, 0)

multiclass_predictions <- multiclass_model %>% predict(test_text)
multiclass_pred_classes <- apply(multiclass_predictions, 1, which.max) - 1
```

```{r}
# Evaluation for Binary Model
binary_metrics <- tibble(
  truth = test_labels_binary,
  predicted = binary_pred_classes
) %>%
...
multiclass_metrics <- tibble(
  truth = test_labels_multiclass,
  predicted = multiclass_pred_classes
) %>%
  metrics(truth = truth, estimate = predicted)
